...
국내 기업을 **CAVM(기업의 질 평가모형)**로 평가하고, 일반기업·금융·보험·반도체의 업종 특성에 맞춘 Final VM과 현재 주가를 분리해 보는 공개 리서치 대시보드입니다.
...
현재 국내 TOP20의 CAVM과 Final VM은 `2026-08-13 공식 마스터`로 승인된 값입니다. `officialFinalVm`은 공개 기준값으로 유지하고 동일 입력으로 재현한 `modelCalculatedVm`을 별도로 보존해 차이를 추적합니다. CAVM 80점 이상을 기본 품질 통과로 보고, 괴리율 -20% 이하 / -20% 초과~-10% 이하 / -10% 초과를 각각 세 구간으로 나눕니다. 공식 마스터 순위의 동점은 승인된 순서를 유지하며, 다음 재선정 때는 해자, 성장성, 현금창출력, 재무건전성, 업종 분산 순으로 검토합니다.
